...
Um arquivo requirements.txt é comumente usado em projetos Python para listar todas as dependências do projeto e suas versões correspondentes. Este arquivo é frequentemente utilizado em conjunto com ferramentas de gerenciamento de pacotes como pip para instalar todas as dependências necessárias para o projeto.

Aqui está um exemplo de como um arquivo requirements.txt pode ser estruturado:
...
numpy==1.19.5: O pacote numpy na versão exata 1.19.5 é necessário.
pandas>=1.2.0: O pacote pandas é necessário, mas qualquer versão igual ou superior a 1.2.0 é aceitável.
matplotlib<3.4: O pacote matplotlib é necessário, mas qualquer versão abaixo da 3.4 é aceitável.
scikit-learn~=0.24.1: O pacote scikit-learn é necessário e a versão deve ser próxima à 0.24.1 (não ultrapassando a versão 0.25).
tensorflow>=2.4,<3.0: O pacote tensorflow é necessário, mas qualquer versão igual ou superior a 2.4 e inferior a 3.0 é aceitável.
Para instalar as dependências listadas em um arquivo requirements.txt, você pode usar o seguinte comando:

installando as dependências 
