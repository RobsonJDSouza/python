...
Os níveis normais de CO2 são em média 430. O nível de CO2 de um local é dado pela média captada pelos 5 sensores.
...
for estado in niveis_co2:
    quantide_co2 = len(niveis_co2[estado])
    soma_co2 = sum(niveis_co2[estado])
    media = soma_co2 / quantide_co2
    if media > 430:
